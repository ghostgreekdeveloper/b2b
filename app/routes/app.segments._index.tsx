...
export const loader = async ({ request }: LoaderArgs) => {
  ...
  const programs = rawPrograms.map((p) => {
    ...
    return {
      ...p,
      cond,
      catalogs: catalogs.filter((c) => c.segmentId === p.id),
    };
  });
...
export const action = async ({ request }: ActionArgs) => {
  ...
  return redirect(`/app/segments/${p.id}`);
};
...
export default function BuyerProgramsPage() {
  ...
  const withCat  = programs.filter((p) => p.catalogs.length > 0).length;

  const filtered = programs.filter((p) => {
    const q = search.toLowerCase();
    return !q || p.title.toLowerCase().includes(q) || p.catalogs.some((c) => c.title.toLowerCase().includes(q));
  });
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            ...
            {filtered.length === 0 && programs.length > 0 && (
              <div style={{ padding: "40px 24px", textAlign: "center" }}>
                <div style={{ fontSize: 14, color: "#94a3b8" }}>No programs match your search.</div>
              </div>
            )}
            ...
            {filtered.map((p, idx) => {
              ...
              return (
                <Link key={p.id} to={`/app/segments/${p.id}`} style={{ textDecoration: "none" }}>
                  <div
                    style={{
                  ...
                  >
                    ...
                    <div style={{
                      ...
                      fontSize: 20, flexShrink: 0, padding: "18px 0",
                      background: m.bg,
                    }}>
                      {m.icon}
                    </div>
                    ...
                    <div style={{ padding: "14px 18px", display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
                      <span style={{
                        ...
                        background: p.catalogs.length > 0 ? "#eff6ff" : "#f1f5f9",
                        color:      p.catalogs.length > 0 ? "#1d4ed8"  : "#94a3b8",
                        border:     `1px solid ${p.catalogs.length > 0 ? "#bfdbfe" : "#e2e8f0"}`,
                        whiteSpace: "nowrap",
                      }}>
                        {p.catalogs.length > 0
                          ? `${p.catalogs.length} catalog${p.catalogs.length !== 1 ? "s" : ""}`
                          : "No catalogs"}
                      </span>
