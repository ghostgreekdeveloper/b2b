...
export const loader = async ({ params, request }: LoaderFunctionArgs) => {
  ...
  const linkedCatalogs = await db.catalog.findMany({
    where: { segmentId },
...
export default function ProgramPage() {
  const { segment, linkedCatalogs, uniqueTags, shopifySegments, dbCustomers, condition } =
            ...
            <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e5e7eb", padding: "20px 22px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <Text as="h2" variant="headingMd">Linked catalogs</Text>
                </div>
                <span style={{
                  fontSize: 11, fontWeight: 700, padding: "2px 10px", borderRadius: 20,
                  background: linkedCatalogs.length > 0 ? "#f0fdf4" : "#f8fafc",
                  color: linkedCatalogs.length > 0 ? "#15803d" : "#64748b",
                  border: `1px solid ${linkedCatalogs.length > 0 ? "#86efac" : "#e2e8f0"}`,
                }}>
                  {linkedCatalogs.length} catalog{linkedCatalogs.length !== 1 ? "s" : ""}
                </span>
              </div>
              <Divider />
              <div style={{ marginTop: 14 }}>
                {linkedCatalogs.length > 0 ? (
                  <BlockStack gap="200">
                    {(linkedCatalogs as { id: number; title: string; status: string }[]).map((cat) => (
                      <div key={cat.id} style={{
                        display: "flex", alignItems: "center", justifyContent: "space-between",
                        padding: "10px 12px", borderRadius: 9,
                        background: cat.status === "active" ? "#f0fdf4" : "#f8fafc",
                        border: `1px solid ${cat.status === "active" ? "#bbf7d0" : "#e2e8f0"}`,
                      }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                          <Badge tone={cat.status === "active" ? "success" : "attention"}>{cat.status}</Badge>
                          <Text as="span" fontWeight="semibold">{cat.title}</Text>
                        </div>
                        <Button variant="plain" url={`/app/catalogs/${cat.id}`}>Open →</Button>
                      </div>
                    ))}
                    <Text as="p" tone="subdued" variant="bodySm">
                      Customers in this program receive prices from all active linked catalogs. Add more by opening a catalog and setting its program to this one.
                    </Text>
                  </BlockStack>
                ) : (
                  <BlockStack gap="200">
                    <Text as="p" tone="subdued">No catalogs linked yet.</Text>
                    <Text as="p" tone="subdued" variant="bodySm">
                      Open a catalog, go to its settings, and assign this program. Multiple catalogs can be linked — customers get all of them.
                    </Text>
